...
const CircleDecorator = ({className, noBorder, scale = 1, blur = 20, opacity = 0.4}: CircleDecoratorProps) => {
  ...
  const clsBorder = cx('border-secondary-40 rounded-full border-[0.0625rem]', {'border-none': noBorder});

  const fontSize = `${56 * scale}rem`;
  ...
  return (
    <div style={{fontSize}} className={cx(`w-[1em] h-[1em] relative`, className)}>
      ...
      <div style={{filter, opacity}} className={cx(clsCenter, clsBorder, 'w-[0.4em] h-[0.4em] bg-secondary-40')} />
    </div>
  );
};
